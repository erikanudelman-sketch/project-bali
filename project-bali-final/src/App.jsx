...
export default function BaliProject() {
  ...
  // Standalone app data is stored locally in this browser/device. Export a
  // backup regularly because local browser data does not automatically sync
  // to another device and can be removed if browser storage is cleared.
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("project-bali-v1");
      if (raw) {
        const saved = JSON.parse(raw);
        if (saved?.setup) setSetup({ ...defaultSetup, ...saved.setup });
        if (saved?.days && typeof saved.days === "object") setDays(saved.days);
        if (Array.isArray(saved?.progressEntries)) setProgressEntries(saved.progressEntries);
        if (Array.isArray(saved?.checkins)) setCheckins(saved.checkins);
        if (Number(saved?.currentDay) >= 1 && Number(saved?.currentDay) <= 50) setCurrentDay(Number(saved.currentDay));
        if (typeof saved?.onboarded === "boolean") setOnboarded(saved.onboarded);
      }
    } catch (e) {
      console.warn("Could not restore local Project Bali data", e);
    } finally {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem("project-bali-v1", JSON.stringify({
        version: 3, onboarded, setup, days, progressEntries, checkins, currentDay
      }));
    } catch (e) {
      console.warn("Could not save local Project Bali data", e);
    }
  }, [storageReady, onboarded, setup, days, progressEntries, checkins, currentDay]);
